# images manage
 ...
 docker rmi [image_name/"IMAGE ID"] #可以跟镜像名字，如果还有其他tag，不会删除镜像，如果删除“IMAGE ID”将会删除所有的"IMAGE ID"的镜像
# 基于本地模板创建镜像
 下载地址：http://openvz.org/Download/templates/precreated
 cat centos-6-x86_64-minimal.tar.gz | docker import - centos-6-x86_64-minimal #import local images
 docker save -o centos-6-x86_64-minimal.tar.gz "REPOSITORY/IMAGE ID" #将现有的镜像导出一个文件，后面可以跟"REPOSITORY"或"IMAGE ID"
 ...
 docker export container_id  > file_name.tar #导出容器，可以迁移到其他机器上，需要先导入为镜像

# continer manage
 docker create  -it  centos #创建一个容器，不启动
 docker start "CONTAINER ID" #启动容器后，可以使用 dockerps  查看到CONTAINER ID
 docker run -i -t centos  bash #使用虚拟中断进入容器，使用命令exit或者ctrl d 退出该bash，当退出后这个容器也会停止。
